...
def read_log(log, jtype, smiles):
    ...
    if jtype == 'neutral':
        ...
        QMs['NPA_spin_density'] = log.NPA_spin_density.astype(np.float) if log.NPA_spin_density is not None else np.nan
        ...
        QMs['homo_1'] = log.homo_1 if log.homo_1 is not None else np.nan 
        QMs['homo_2'] = log.homo_2 if log.homo_2 is not None else np.nan
        QMs['lumo'] = log.lumo if log.lumo is not None else np.nan
        QMs['lumo_1'] = log.lumo_1 if log.lumo_1 is not None else np.nan
        QMs['lumo_2'] = log.lumo_2 if log.lumo_2 is not None else np.nan

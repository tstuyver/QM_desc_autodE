...
class G16Log:
    ...
    def GetNPA(self):
        ...
        # NPA charge and spin preliminaries
        NPA_Charge = np.zeros([len(self.AtomsNum), 3])
        NPA_charge_alpha = np.zeros(len(self.AtomsNum))
        ...
        if self.mult != 1:
            ...
            for i in range(len(NPA_spin)):
                NPA_spin[i] = NPA_charge_beta[i] - NPA_charge_alpha[i]

        self.NPA_spin_density = NPA_spin
            ...
            if line.find('Alpha  occ.') > -1:
                ...
                if float(m[-1]) > homo:
                    ...
                    if len(m) >= 3:
                        ...
                        homo_1 = float(m[-2])
                        homo_2 = float(m_previous_line[-1])
                    else:
